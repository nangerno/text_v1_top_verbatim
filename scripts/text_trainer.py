...
def main():
    ...
    parser.add_argument("--reset-state", action="store_true", help="Reset state from previous run")
    ...
    train_success = False
    # ...
    # Initialize / restore state machine.
    # NOTE: training scripts (via CustomEvalSaveCallback) may set state["mode"]="continue"/"finish".
    if args.reset_state:
        state = {}
    else:
        state = get_state() or {}

    # Only default to "initial" if caller/training hasn't already decided a mode.
    ...
    count = 0
    while True:
        # Safety: don't start a new run if we're too close to the deadline.
        # (Training scripts will also checkpoint based on end_time, but we avoid launching new runs.)
        try:
            end_time_obj = datetime.strptime(end_time, "%Y-%m-%d %H:%M:%S").replace(
                tzinfo=timezone.utc
            )
            now_utc = datetime.now(timezone.utc)
            if (end_time_obj - now_utc).total_seconds() < 120:
                print(
                    "Less than 2 minutes remaining until end_time; stopping new training runs.",
                    flush=True,
                )
                break
        except Exception as e:
            print(f"Warning: failed to parse/compare end_time ({end_time}): {e}", flush=True)
        ...
        if args.task_type == TaskType.GRPOTASK.value:
            ...
            if state["mode"] == "initial":
                ...
                if not n_runs or n_runs < 1:
                    raise ValueError(f"Invalid next_runs: {n_runs}")
                ...
                if len(state["runs"]) < n_runs:
                    ...
                    if index >= len(state["lrs"]):
                        raise ValueError(f"Index {index} out of bounds for lrs list of length {len(state['lrs'])}")
