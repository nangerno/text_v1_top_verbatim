...
class CustomEvalSaveCallback(TrainerCallback):
    ...
    def on_step_end(self, args, state: TrainerState, control: TrainerControl, **kwargs):
        ...
        if state.global_step == self.checking_step and self.checking_mode == "first_time":
            ...
            my_state = get_state()
            if "train" not in my_state or "start_time" not in my_state["train"]:
                print("Warning: Missing start_time in state, using current time", flush=True)
                my_state.setdefault("train", {})["start_time"] = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            ...
            if self.checking_step <= 0:
                raise ValueError(f"checking_step must be positive, got {self.checking_step}")
            ...
            all_runs = my_state["runs"] + [{"current_loss": current_loss}]
            if not my_state.get("runs"):
            ...
            if current_loss <= current_min_loss:
                if len(my_state["runs"]) + 1 == my_state["next_runs"]:
                    print(f"Current loss: {current_loss} is the best (<= {current_min_loss}), continuing training", flush=True)
